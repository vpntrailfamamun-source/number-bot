...
from aiogram.types import Message, CallbackQuery, KeyboardButton, InlineKeyboardButton, Document
...
async def use_num(c: CallbackQuery):
    await c.answer()
...
async def handle_text(m: Message):
    if m.from_user.id != ADMIN or m.from_user.id not in admin_state:
        return
    ...
    if step == "nums":
        ...
        if nums:
            admin_state[m.from_user.id]["nums"] = nums
            admin_state[m.from_user.id]["step"] = "svc"
            await m.answer(f"✅ *{len(nums)}* টি নাম্বার পাওয়া গেছে।\n\nসার্ভিসের নাম পাঠান (যেমন: WhatsApp, Facebook):", parse_mode="Markdown")
        else:
        ...
        if m.text.strip().lower() == "yes":
            ...
            added   = 0
            skipped = 0
            for num in admin_state[m.from_user.id]["nums"]:
                flag, country, iso = get_country_info(num)
                try:
...
# ================== OTP গ্রুপ অটো ফরওয়ার্ড ==================
def match_number(group_text, real):
    real_digits = re.sub(r'\D', '', real)
    blocks = re.findall(r'\d+', group_text)
    for block in blocks:
        if len(block) >= 4:
            if real_digits.startswith(block[:4]) and real_digits.endswith(block[-4:]):
                return True
    return False

@dp.message(F.chat.id == OTP_GROUP_ID)
async def otp_group_handler(m: Message):
    ...
    for w_id, user_id, number, service, flag, iso in waiters:
        if match_number(text, number):
            line = "\u2500" * 30
            msg  = f"`{flag}{service} \u2022 {iso}`\n`{number}`\n`{line}`\n`          {otp}          `\n`{line}`"
            kb   = InlineKeyboardBuilder()
            kb.row(InlineKeyboardButton(text=f"\U0001f511 {otp}", callback_data="noop", style="success"))
            try:
